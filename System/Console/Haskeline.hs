...
getInputLineWithInitial ::
  ...
  (String, String) ->
  InputT m (Maybe String)
getInputLineWithInitial prompt (left, right) =
  promptedInput
    (getInputCmdLine initialIM)
...
maybeAddHistory result = do
  ...
  case result of
    Just line
      | autoAddHistory settings && not (all isSpace line) ->
  ...
  where
    loop =
      choiceCmd
        [ simpleChar '\n' +> finish,
          simpleKey Backspace
            +> change deletePasswordChar
            >|> loop',
          useChar $ \c -> change (addPasswordChar c) >|> loop',
          ctrlChar 'd' +> \p ->
            if null (passwordState p)
              then failCmd p
              else finish p,
