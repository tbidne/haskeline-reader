...
stringToGraphemes = mkString . dropWhile isCombiningChar
  where
    ...
    mkString ('\ESC' : cs)
      | (ctrl, '\STX' : rest) <- break (== '\STX') cs =
          Grapheme '\ESC' ctrl : stringToGraphemes rest
    mkString (c : cs) =
      Grapheme c (takeWhile isCombiningChar cs)
...
skipLeft f (IMode xs ys) =
  let (ws, zs) = span (f . baseChar) xs
   in IMode zs (reverse ws ++ ys)
skipRight f (IMode xs ys) =
  let (ws, zs) = span (f . baseChar) ys
...
instance (LineState s) => LineState (ArgMode s) where
  beforeCursor _ am =
    let pre = map baseGrapheme $ "(arg: " ++ show (arg am) ++ ") "
