...
pureMovements = choiceCmd $ charMovements ++ map mkSimpleCommand movements
  where
    charMovements =
      [ charMovement 'f' $ \c -> goRightUntil $ overChar (== c),
...
useMovementsForKill alternate useHelper =
  choiceCmd $
    specialCases
      ++ map (\(k, move) -> k +> useHelper (SimpleMove move)) movements
  where
    specialCases =
      [ simpleChar 'e' +> useHelper (SimpleMove goToWordDelEnd),
...
foreachDigit f ds = choiceCmd $ map digitCmd ds
  where
    digitCmd d = simpleChar d +> change (f (toDigit d))
...
deleteMatchingBrace (IMode xs (y : ys))
  | Just b <- matchingRightBrace yc,
    Just (bs, ys') <- scanBraces yc b ys =
      (y : reverse bs, IMode xs ys')
  | Just b <- matchingLeftBrace yc,
    Just (bs, xs') <- scanBraces yc b xs =
      (bs ++ [y], IMode xs' ys)
  where
