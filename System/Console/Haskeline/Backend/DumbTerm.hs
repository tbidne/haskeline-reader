...
data Window = Window
  { -- | # of visible chars to left of cursor
    pos :: Int
  }
...
drawLineDiff' (xs1, ys1) (xs2, ys2) = do
  ...
  if xw1 > p || newP >= w
    then refitLine (xs2, ys2)
      ...
      -- we haven't moved outside the margins
      put Window {pos = newP}
      case (xs1', xs2') of
        ([], []) | ys1 == ys2 -> return () -- no change
        (_, [])
          | xs1' ++ ys1 == ys2 -> -- moved left
              printText $ backs xw1
        ([], _)
          | ys1 == xs2' ++ ys2 -> -- moved right
              printText (graphemesToString xs2')
        _ ->
          let extraLength =
                xw1
                  + snd (takeWidth (w - p) ys1)
                  - xw2
                  - yw2
           in printText $
                backs xw1
                  ++ graphemesToString (xs2' ++ ys2')
                  ++ clearDeadText extraLength
                  ++ backs yw2
